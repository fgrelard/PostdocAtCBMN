...
Other techniques try to estimate the noise level in the image, so as to predict the amount of signal overlap between pixels. Some methods in [[doi:10.1039/C004323K][[2]​]] deduce a function mapping a binary input image to the associated output with contrast variations. 
