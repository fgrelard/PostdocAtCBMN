...
class WorkerRegistrationSelection(QtCore.QObject):
    ...
    def work(self):
        print("starting")
        fixed_itk = sitk.GetImageFromArray(self.fixed.T)
        moving_itk = sitk.GetImageFromArray(self.moving.T)
        ...
        print(moving_itk.GetSpacing(), fixed_itk.GetSpacing())
        if dim == 2:
        ...
        deformed = sitk.GetArrayFromImage(deformed_itk).T
