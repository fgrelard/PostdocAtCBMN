...
_State-of-the-art:_
1. ~Cardinal~
   Read images on the fly without loading the whole image in memory. Access for one ion image is done through the ~imzML~ file.
...
2. ~MSiReader~
   Creates a ~mat~ file containing various information:
   - ~ibdptrs~: 2D array of indices (~n_spectral x 2~), begin  indices in the ~ibd~ file for each peak, for /m/z/ and intensity values
   - ~index~: array of ones
   - ~loc~: 2D array of indices (~width x height~), pixel positions to flattened indices
   
_TODOs:_
**** TODO Find efficient methods in the literature or in alternative softwares
:LOGBOOK:
