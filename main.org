...
:END:
**** TODO Implementation of memory-mapped files for ~imzML~ enclosing sparse matrices (centroided data) [0/1]
***** TODO 

**** TODO Implementation of on-the-fly read in  ~MSImageImplementation~, similarly to ~MSiReader~.
*** ~imzML~ files
_TODOs_:
