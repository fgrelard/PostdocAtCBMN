...
1. ~Cardinal~
   ...
   ~readMSIData~: Read images on the fly without loading the whole image in memory. Access for one ion image is done through the ~imzML~ file. [[http://bioconductor.org/packages/release/bioc/manuals/Cardinal/man/Cardinal.pdf][Manual.]]
   _Pros:_
...
2. ~MSiReader~
   ...
   Creates a ~mat~ file (extension ~.ibh~) containing various information and allowing for faster reloading of data.
