...
Registration is usually the first step of multimodal image integration. It consists in aligning two or more images, such that the enclosed objects are superimposed. Registration methods aim at estimating the transformation which maps a moving, or deformable image onto a fixed, or reference image. They can be classified in two categories, depending on the information they use: (a) landmarks, or (b) pixel intensities.
Landmarks are features in the image that are salient and easily recognizable. They can be selected manually or automatically. For instance, [[cite:&Borodinov_2020]] register MALDI images onto a SIMS image by selecting matching pairs of fiducial markers. Automatic methods identify and match landmark points based on local image descriptors, such as SIFT ([[cite:&Lowe_1999]]) or SURF ([[cite:&Bay_2006]]). These methods are generally not suited to a multimodal task involving images of different nature.
