...
Class discovery can be achieved by data dimension reduction methods or clustering (see Section [[*Data analysis/statistics][Data analysis/statistics]]). [[cite:&Abdelmoula_2019]] find segmentation maps of registered MALDI-MS images with the hierarchical variant of the t-SNE algorithm. Then, spatial correlations are established by Pearson correlation coefficient between the segmentation map and the ion images. [[cite:&Verbeeck_2017]] automatically identify the spatial distributions observed in registered ion images as a combination of the anatomical structures described in the atlas built from MR images. Then, they use Wilcoxon rank test to find differently expressed ions between healthy and diseased brain hemispheres. [[cite:&Patterson_2018]] find spatial correlations between two MS images by weighting correlations by their degree of pixel overlap. [[cite:&Ryabchykov_2018]]  Raman and MALDI: by PCA on weighted data to attribute the same importance to both datasets.
