...
The large dataset size makes the hyperspectral datasets difficult to analyze in a multimodal setting. Various approaches are designed to reduce the complexity of the datasets. For instance, peak picking and alignment reduce the dataset size while preserving the relevant information. The dataset can be further reduced by dimension reduction techniques, such as PCA, NMF (non-negative matrix factorization) or t-SNE ([[cite:&Vandermaaten_08a]]). PCA and NMF are linear matrix decomposition techniques, whereas t-SNE is a probabilistic, non-linear dimension reduction technique. PCA and NMF are matrix decomposition techniques where the original, large, MALDI-MS image is approximated by the product of two smaller matrices. Common global information across ion images are stored in so-called component images. The component images usually highlight important structures in the tissue. NMF impose a non-negativity constraint on the components which makes them easier to interpret. By contrast, t-SNE retains local structures by preserving distances between points in a lower-dimensional embedding. The resulting reduction can make for 
