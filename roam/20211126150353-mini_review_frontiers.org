...
Landmarks are features in the image that are salient and easily recognizable. They can be selected automatically or manually.Automatic methods identify and match landmark points based on local image descriptors, such as SIFT ([[cite:&Lowe_1999]]) or SURF ([[cite:&Bay_2006]]). These methods are generally not suited to a multimodal task since the images enclose different types of information.
[[cite:&Borodinov_2020]] register MALDI images onto a SIMS image by manually selecting matching pairs of fiducial markers. 
