...
Methods based on pixel intensity similarities are automatic. The transformation parameters are modified iteratively until a local optimum of the similarity metric is reached.  The similarity metric measures the matching of two images after applying a deformation. Typical similarity metrics are the sum of squared differences (SSD) over both images, or normalized cross-correlation. The SSD does not account for contrast similarities between images, as opposed to normalized cross-correlation.
