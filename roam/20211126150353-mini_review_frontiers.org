...
Before registration, images should be comparable and have the same number of dimensions. This implies that hyperspectral images must be processed. Involved images can be used directly (e.g. ion image in MALDI) or come from a pre-processing step, such as a component image from dimension reduction techniques, or a cluster image (see Section [[*Data analysis/statistics][Data analysis/statistics]]). Segmentation methods extract objects of interest in the images. They can be based on intensities, the geometrical properties of the objects or a combination of both. Intensity-based methods Simple thresholding ([[cite:&Anyz_2017]]), region growing
