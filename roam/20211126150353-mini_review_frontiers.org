...
The last step of a multimodal workflow is usually the joint statistical analysis of the images. This involves using the information from both modalities to evidence complementarities and correlations. The algorithms depend on the scientific goal: (1) class discovery to find commonalities without any /a priori/ information, (2) class prediction, using existing information to infer spatial or biological knowledge in a new dataset. Extraneous information usually comes from expert annotations or from regions found in the complementary modality

Class discovery can be achieved by data dimension reduction methods or clustering (see Section [[*Data analysis/statistics][Data analysis/statistics]]). [[cite:&Abdelmoula_2019]] find segmentation maps of registered MALDI-MS images by a hierarchical variant of the t-SNE algorithm. Then, spatial correlations are established by Pearson correlation coefficient between the segmentation map and the ion images.  [[cite:&Ryabchykov_2018]]  find spatial correlations between Raman and MALDI-MS images by analyzing PCA scores and loadings. This analysis highlights changes in lipid distributions.g

Class prediction can be achieved by supervised methods, that is to say methods that use annotated datasets. [[cite:&Borodinov_2020]] obtain MALDI-MS images with high resolution by combining them with TOF-SIMS images. They use Canonical Correlation Analysis (CCA) on NMF component images to find similarities. This decomposition and transformation enact the reconstruction of MALDI ion images from the high-resolution TOF-SIMS images.
...
[[cite:&Bemis_2019]]
 [[cite:&Verbeeck_2017]] automatically identify the spatial distributions observed in registered ion images as a combination of the anatomical structures described in the atlas built from MR images. Then, they use Wilcoxon rank test to find differently expressed ions between healthy and diseased brain hemispheres.

 [[cite:&Patterson_2018]] find spatial correlations between two MS datasets by weighing abundance correlations by their degree of pixel overlap.
