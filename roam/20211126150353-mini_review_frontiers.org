...
Registration is usually the first step of multimodal image integration. It consists in aligning two or more images, such that the enclosed objects are superimposed. Registration methods aim at estimating the transformation which maps a moving, or deformable image onto a fixed, or reference image. The transformation can be rigid (translation and rotation), affine (translation, rotation, scaling, and shearing) or nonlinear. Nonlinear models account for local deformations in the image. They are necessary to accurately model the deformations induced during sample preparation, such as slicing or enzymatic digestion of the tissue.
...
Landmarks are features in the image that are salient and easily recognizable. They can be selected manually or automatically. For instance, [[cite:&Borodinov_2020]] register MALDI images onto a SIMS image by selecting matching pairs of fiducial markers. Automatic methods identify and match landmark points based on local image descriptors, such as SIFT ([[cite:&Lowe_1999]]) or SURF ([[cite:&Bay_2006]]). These methods are generally not suited to a multimodal task since the images represent different types of information.



Convention is that the image with the higher
spatial resolution takes the role of the fixed image

Transformation models can be rigid, non-rigid
(affine), projective, or nonlinear
