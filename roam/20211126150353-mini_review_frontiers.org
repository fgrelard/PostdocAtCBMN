...
As a last step of data analysis, univariate analysis can be used to find statistical differences between spatial distributions of molecules, or regions in the image. The choice of the method depends on the MSI dataset and the hypothesis of the experiment. T-test is used to highlight statistical differences for a specific m/z between two different samples [[cite:&Yajima_2018]], or regions within a tissue section, whereas the analysis of variance (ANOVA) test can be used to compare three or more regions [[cite:&Blanc_2018]]. When the normality assumption is not verified, as is often the case for MSI datasets, non-parametric tests, such as Mann-Whitney U-test or Wilcoxon signed-rank tests can be used [[cite:&Guo_2014]]. To determine if an ion is a good biomarker to distinguish different conditions, ROC curve analysis can be applied [[cite:&Hoo_2017]].

** Pre-processing
Before registration, images should be comparable and have the same number of dimensions. Standardization is used such that the images are in the same intensity range. Involved images can be used directly (e.g. ion image in MALDI) or come from segmentation methods, which extract objects of interest in images. For instance, segmentation maps from dimension reduction techniques (e.g. component image), or clustering, can be used (see Section [[*Data analysis/statistics][Data analysis/statistics]]).
...
The last step of a multimodal workflow is usually the joint statistical analysis of the images. This involves using the information from both modalities to evidence complementarities and correlations. The algorithms can be grouped according to the scientific goal: (1) class discovery, i.e. find commonalities without any /a priori/ information, (2) class prediction, using existing information to infer spatial or biological knowledge in a new dataset, and (3) class comparison, using extraneous information to find differentially abundant ions. The extraneous information usually comes from expert annotations or from regions found in the complementary modality. Class discovery is usually followed by class comparison.

Class discovery can be achieved by data dimension reduction methods or clustering (see Section [[*Data analysis/statistics][Data analysis/statistics]]).

[[cite:&Abdelmoula_2019]] find segmentation maps of registered MALDI-MS images with the hierarchical variant of the t-SNE algorithm. Then, correlations are established by Pearson correlation coefficient between the segmentation map and the ion images. Those with higher Pearson coefficient are spatially distributed within this cluster.



[[cite:&Borodinov_2020]] TOF+MALDI: pan sharpening by NMF + CCA (Canonical correlation analysis)

[[cite:&Abdelmoula_2019]] Segmentation maps are obtained by [[id:e48f89e0-263b-4dbe-ab29-70bd981d2312][HSNE]], a hierarchical version of the t-[[id:e95fd377-dd05-4363-8eca-b00b003524ff][SNE]].
...
[[cite:&Verbeeck_2017]] Wilcoxon rank sum test to find differently expressed ions over healthy and diseases brain hemispheres.
...
[[cite:&Bemis_2019]] class comparison, design different models based on hierarchical Bayesian spatial models.
[[cite:&Ryabchykov_2018]]  Raman and MALDI: by PCA on weighted data to attribute the same importance to both datasets.
Both data highlight the change in lipid distribution (third PCA component) + correlation map
[[cite:&Wehrli2020-spatial]] Multiblock class prediction
[[cite:&Patterson_2018]] Overlap (definition?) is computed to establish [[id:8d07df0d-3aac-4797-8253-751aa8bfd7b7][spatial correlations]]. Ions are ranked according to their degree of overlap.
