...
* Writing
** Pre-processing
The purpose of pre-processing is to obtain images which are comparable in different modalities, so as to facilitate the registration process.
In this section, we focus on methods which are compatible with MALDI-MSI datasets.
