...
Iconic methods are used in numerous MALDI-MSI registration tasks. [[cite:&Castellanos_Garcia_2021]] combine MALDI-MS with LA-ICP-MS images, so as to merge the extensive molecular information from MALDI with the better signal quality in LA-ICP-MS images. They select matching ion images manually in order to compute both linear and non-linear transformations.
[[cite:&Abdelmoula_2019]]  register several 2D MALDI slices with a 3D MR image in order to obtain a volumetric representation of the MALDI dataset. The MALDI are reduced by the t-SNE algorithm
