...
Methods based on pixel intensity similarities are automatic. The transformation parameters are modified iteratively until a local optimum of the similarity metric is reached.  These methods involve three main components: (1) the transformation model, (2) the similarity metric and (3) the optimization algorithm.  The similarity metric measures the matching of two images after applying a deformation. Typical similarity metrics are the sum of squared differences (SSD)
