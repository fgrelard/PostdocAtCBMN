...
The last step of a multimodal workflow is usually the joint statistical analysis of the images. This involves using the information from both modalities to evidence complementarities and correlations. The algorithms can be grouped according to the scientific goal: (1) class discovery, i.e. find commonalities without any /a priori/ information, (2) class prediction, using existing information to infer spatial or biological knowledge in a new dataset, and (3) class comparison, using extraneous information to find differentially abundant ions. The extraneous information usually comes from expert annotations or from regions delineated in the complementary modality. Class discovery and predictions are usually followed 
