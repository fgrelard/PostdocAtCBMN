...
The variety of processing methods makes it difficult to choose them appropriately. [[cite:&Abdelmoula_2022]] use a deep learning architecture to classify tumor and healthy pixels in an MS dataset.  a lower, non-linear representation of the MSI dataset from . which works with the full spectra, without peak picking or alignment. 
