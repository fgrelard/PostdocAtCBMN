...
[[cite:&Abdelmoula_2019]]  register several 2D MALDI slices with a 3D MR image in order to obtain a volumetric representation of the MALDI dataset. The MALDI image is reduced by the t-SNE algorithm and represents the moving image.  The corresponding 2D MR image is selected from the 3D volume manually. Then, both a linear and non-linear transformation model is applied 
