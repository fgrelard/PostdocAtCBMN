...
The quality of the registration can be evaluated by various metrics, such as the Dice coefficient, or the F-measure, which estimate the proportion of overlapping pixels between the two images after registration. When using non-linear transformations, it is important to obtain a good compromise between shape matching and intensity fidelity. For instance, [[cite:&Grelard_2021]] estimate the intensity fidelity by computing the mutual information between both images.
