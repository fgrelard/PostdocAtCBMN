...
Before registration, images should be comparable and have the same number of dimensions. This implies that hyperspectral images must be processed. Involved images can be used directly (e.g. ion image in MALDI) or come from a pre-processing step: component image from dimension reduction techniques, cluster image (see Section [[*Data analysis/statistics][Data analysis/statistics]]). Other traditional or modality-specific segmentation algorithms can be used.
