...
As a last step of data analysis, univariate analysis can be used to find statistical differences between spatial distributions of molecules, or regions in the image. The choice of the method depends on the MSI dataset and the hypothesis of the experiment. T-test is used to highlight statistical differences for a specific m/z between two different samples [[cite:&Yajima_2018]], or regions within a tissue section, whereas the analysis of variance (ANOVA) test can be used to compare three or more regions [[cite:&Blanc_2018]]. When the normality assumption is not verified, as is often the case for MSI datasets, Mann-Whitney U-Test should be used [[cite:&Guo_2014]]. To determine if an ion is a good biomarker to distinguish different conditions, ROC curve analysis can be applied [[cite:&Hoo_2017]].

** Pre-processing
There are multiple methods for pre-processing the images, which are often dependent on the modality. In this section, we focus on common processing tasks which facilitate the registration process.
...
Segmentation methods are usually based on pixel intensities or use the geometrical properties of the objects. In a multimodal setting involving MALDI-MSI, common methods involve simple thresholding ([[cite:&Anyz_2017]]), region growing ([[cite:&Grelard_2021]]), methods assisting in selecting spatially coherent ion images ([[cite:&Alexandrov_2013]]), or machine learning
