...
Denoising is commonly applied to remove spurious signal from images. Generic methods, such as median or gaussian filtering, can help to get rid of general inten
MALDI-MS images are difficult to analyze in a multimodal setting due to the large dataset size. Various approaches are designed to reduce their complexity.
For instance, peak picking and alignment reduce the dataset size while preserving relevant information.
The dataset can be further reduced by dimension reduction techniques, such as PCA, NMF (non-negative matrix factorization) or t-SNE ([[cite:&Vandermaaten_08a]]). PCA and NMF are linear matrix decomposition techniques, whereas t-SNE is a probabilistic, non-linear dimension reduction technique. PCA and NMF are matrix decomposition techniques where the original, large, MALDI-MS image is approximated by the product of two smaller matrices. Common global information shared across ion images are stored in so-called component images. The component images usually highlight important structures in the tissue. NMF impose a non-negativity constraint on the components which makes them easier to interpret. By contrast, t-SNE retains local structures by preserving distances between points in a lower-dimensional embedding. This reduction makes for visualizations that are faithful to the original image data. However, the resulting mapping is not deterministic, is harder to explain and the required computational resources are significantly larger than for PCA. For MALDI-MS images, it is often required to apply PCA so as to reduce the dimensionality, before using t-SNE. [[cite:&Abdelmoula2018-inter]] used a variant of t-SNE, called hierarchical SNE ([[cite:&Pezzotti_2016]]), to visualize structures with different levels of details across ion images.

Clustering methods give simplified representations of the datasets, where similar spectra are grouped together. The output of clustering can be visualized as a 2D image where each pixel has an intensity equal to its cluster number.
