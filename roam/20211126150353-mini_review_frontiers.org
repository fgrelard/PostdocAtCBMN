...
Class prediction can be achieved by supervised methods, that is to say methods that use annotated datasets. [[cite:&Borodinov_2020]] obtain MALDI-MS images with high resolution by combining them with TOF-SIMS images. They use Canonical Correlation Analysis (CCA) on NMF component images to find similarities. This decomposition and transformation enact the reconstruction of MALDI ion images from the high-resolution TOF-SIMS images.  [[cite:&Wehrli2020-spatial]] combine Raman and MALDI-MS images and extract a high-resolution MALDI-MS image. Since both images are hyperspectral, they use a multiblock method derived from Orthogonal PLS.
